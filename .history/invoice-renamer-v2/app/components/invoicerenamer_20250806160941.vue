...
const invoiceData = shallowRef<InvoiceData[]>([])
...
const handleFileChange = async (file: any, fileList: any[]) => {
  ...
  setTimeout(async () => {
    ...
    if (!batchProcessing.value) {
      ...
      console.log(`🚀 启动处理模式，新文件数: ${actualNewFileCount}, 总文件数: ${fileList.length}`)
...
const removeFile = (index: number) => {
  console.log('🗑️ 删除文件，索引:', index, '当前数据长度:', invoiceData.value.length)
