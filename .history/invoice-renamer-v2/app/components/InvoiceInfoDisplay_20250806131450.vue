      ...
      <template #header>
        <div class="flex justify-between items-center">
          <div class="flex items-center gap-3">
            ...
            <span
              class="px-2 py-1 bg-blue-50 text-blue-600 text-sm rounded-full"
        ...
        <div class="w-full overflow-visible">
          ...
          >
            ...
            <el-table-column type="expand" width="50">
              <template #default="{ row }">
                <div class="p-5 bg-gray-50 rounded-lg my-2">
                  <div class="mb-5">
                    <h4
                      class="flex items-center gap-2 mb-3 text-gray-800 text-sm font-semibold m-0"
                    >
                      详细信息
                    </h4>
                    <div
                    ...
                    >
                      <div class="flex items-center gap-2 text-xs py-1">
                        <label
                          class="font-medium text-gray-600 min-w-20 flex-shrink-0"
                          >购买方税号:</label
                        >
                      ...
                      <div class="flex items-center gap-2 text-xs py-1">
                        ...
                        >
                        <span class="text-gray-800 flex-1">{{
                          row.taxAmount ? `¥${row.taxAmount}` : "未识别"
                        }}</span>
                      </div>
                      <div class="flex items-center gap-2 text-xs py-1">
                        <label
                  ...
                  <div class="mb-0">
                    ...
                    <div
                      class="bg-white border border-gray-200 rounded-md p-3 text-xs leading-relaxed text-gray-600 max-h-50 overflow-y-auto whitespace-pre-wrap break-all"
                    >
                      {{ row.fullText || "无文本内容" }}
                    </div>
                  </div>
              ...
              <template #default="{ row }">
                <div class="flex items-center py-1">
                  <el-tooltip :content="row.fileName" placement="top">
                    <span class="text-sm text-gray-800 truncate max-w-full">{{
                      row.fileName
                    }}</span>
                  </el-tooltip>
                </div>
            ...
            >
              <template #default="{ row }">
                <div class="flex items-center py-1">
                  <span v-if="row.invoiceDate" class="text-sm text-gray-800">{{
                    row.invoiceDate
                  }}</span>
                  <span v-else class="text-sm text-gray-400">未识别</span>
                </div>
            ...
            <el-table-column label="操作" width="200" fixed="right">
              <template #default="{ row, $index }">
                <div
                  class="flex items-center justify-center gap-2 py-1 whitespace-nowrap"
                >
                  <el-button
                    type="primary"
                    size="small"
      ...
      >
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          >
            <!-- 发票预览图 -->
            <div
              class="relative h-40 bg-gradient-to-br from-blue-50 to-blue-100 flex items-center justify-center cursor-pointer overflow-hidden"
              @click="handlePreviewClick(invoice.file)"
            >
              <div
                class="flex flex-col items-center gap-2 text-blue-600 transition-all duration-300"
              >
                <el-icon class="text-3xl"><Document /></el-icon>
                <span class="text-sm font-medium">点击预览</span>
              </div>
              <div
                class="absolute inset-0 bg-black bg-opacity-60 flex items-center justify-center opacity-0 hover:opacity-100 transition-opacity duration-300"
              >
            ...
            <div class="p-4">
              <div class="flex justify-between items-start mb-3">
                <h3
                  class="text-base font-semibold text-gray-800 m-0 flex-1 mr-2 leading-tight"
              ...
              <div class="flex flex-col gap-2 mb-3">
                <div class="flex items-center gap-2 text-xs">
                  <label
                    class="font-medium text-gray-600 min-w-16 flex-shrink-0"
                    >发票号码:</label
                  >
                  <span
                ...
                <div class="flex items-center gap-2 text-xs">
                  ...
                  <span
                    ...
                    class="text-gray-800 flex-1 truncate"
                ...
                <div
                  class="flex items-center gap-2 text-xs p-2 bg-green-50 border border-green-200 rounded"
                >
                  <label
                    class="font-medium text-gray-600 min-w-16 flex-shrink-0"
                    >价税合计:</label
                  >
                  <span
                    v-if="invoice.totalAmount"
                    class="font-semibold text-green-600 text-sm flex-1"
      ...
      <div v-if="selectedInvoice" class="w-full">
        ...
        >
          ...
          <div class="flex items-center gap-2">
            <el-button
              type="primary"
              @click="downloadRenamedFile(selectedInvoice)"
            >
        ...
        <div class="space-y-6">
          <div class="bg-gray-50 rounded-lg p-4">
            <h4 class="text-base font-semibold text-gray-800 mb-4 m-0">
              发票信息
            </h4>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              <div class="flex items-center gap-2 text-sm py-1">
                <label class="font-medium text-gray-600 min-w-20 flex-shrink-0"
                  >发票号码:</label
                >
                <span class="text-gray-800 flex-1">{{
                  selectedInvoice.invoiceNumber || "未识别"
                }}</span>
              </div>
              <div class="flex items-center gap-2 text-sm py-1">
                <label class="font-medium text-gray-600 min-w-20 flex-shrink-0"
                ...
                >
                <span class="text-gray-800 flex-1">{{
                  selectedInvoice.invoiceDate || "未识别"
                }}</span>
              </div>
              <div class="flex items-center gap-2 text-sm py-1">
                <label class="font-medium text-gray-600 min-w-20 flex-shrink-0"
                  >价税合计:</label
                >
                <span class="text-blue-600 font-medium flex-1">{{
              ...
              <div class="flex items-center gap-2 text-sm py-1">
                ...
                >
                <span class="text-gray-800 flex-1">{{
                  selectedInvoice.sellerName || "未识别"
                }}</span>
              </div>
              <div class="flex items-center gap-2 text-sm py-1">
                <label class="font-medium text-gray-600 min-w-20 flex-shrink-0"
                ...
                >
                <span class="text-gray-800 flex-1">{{
                  selectedInvoice.buyerTaxId || "未识别"
                }}</span>
              </div>
              <div class="flex items-center gap-2 text-sm py-1">
                <label class="font-medium text-gray-600 min-w-20 flex-shrink-0"
                ...
                >
                <span class="text-gray-800 flex-1">{{
                  selectedInvoice.sellerTaxId || "未识别"
                }}</span>
              </div>
              <div class="flex items-center gap-2 text-sm py-1">
                <label class="font-medium text-gray-600 min-w-20 flex-shrink-0"
              ...
              <div class="flex items-center gap-2 text-sm py-1">
                ...
                >
                <span class="text-gray-800 flex-1">{{
                  selectedInvoice.amountWithoutTax || "未识别"
                }}</span>
              </div>
              <div class="flex items-center gap-2 text-sm py-1">
                <label class="font-medium text-gray-600 min-w-20 flex-shrink-0"
          ...
          <div class="bg-gray-50 rounded-lg p-4">
            ...
            <div
              class="bg-white border border-gray-200 rounded-md p-4 text-sm leading-relaxed text-gray-600 max-h-80 overflow-y-auto whitespace-pre-wrap break-all"
            >
              {{ selectedInvoice.fullText || "无原始文本" }}
            </div>
          </div>
        </div>
      </div>
...
const downloadRenamedFile = (invoice: InvoiceData) => {
  // 生成重命名的文件名
  const date = invoice.invoiceDate || "未知日期"
  const seller = invoice.sellerName || "未知销售方"
  const amount = invoice.totalAmount || "未知金额"
  const invoiceNum = invoice.invoiceNumber || "未知号码"

  const newFileName = `${date}_${seller}_${amount}_${invoiceNum}.pdf`
