<template>
  <div class="invoice-info-display">
    <el-card class="display-card" shadow="hover">
      ...
      <div v-if="viewMode === 'table'" class="table-view">
        <div class="table-wrapper">
          ...
          >
                ...
                <div class="expand-content">
                  <div class="expand-section">
                    ...
                    <div class="info-grid">
                      ...
                      <div class="info-item">
                        <label>税额:</label>
                        <span>{{ row.taxAmount ? `¥${row.taxAmount}` : '未识别' }}</span>
                      </div>
                      <div class="info-item">
                        <label>不含税金额:</label>
                        <span>{{ row.amountWithoutTax ? `¥${row.amountWithoutTax}` : '未识别' }}</span>
                      </div>
                      <div class="info-item">
                        <label>开票人:</label>
                  ...
                  <div class="expand-section">
                    <h4>原始文本 <el-tag size="small" type="info">{{ row.parseMethod || 'PDFToWordParser' }}</el-tag></h4>
                    <div class="full-text">
                      {{ row.fullText || '无文本内容' }}
                    </div>
                  </div>
            ...
            <!-- 文件名 -->
            <el-table-column prop="fileName" label="文件名" min-width="200" show-overflow-tooltip sortable>
              <template #default="{ row }">
            ...
            <!-- 发票号码 -->
            <el-table-column prop="invoiceNumber" label="发票号码" min-width="180" show-overflow-tooltip sortable>
              <template #default="{ row }">
                <span v-if="row.invoiceNumber" class="invoice-number">{{ row.invoiceNumber }}</span>
            ...
            <el-table-column prop="invoiceDate" label="开票日期" min-width="120" sortable>
              <template #default="{ row }">
                <span v-if="row.invoiceDate" class="invoice-date">{{ row.invoiceDate }}</span>
                <el-tag v-else type="warning" size="small">未识别</el-tag>
              </template>
            </el-table-column>

            <!-- 购买方 -->
            <el-table-column prop="buyerName" label="购买方" min-width="150" show-overflow-tooltip>
              <template #default="{ row }">
                <span v-if="row.buyerName" class="buyer-name">{{ row.buyerName }}</span>
                <el-tag v-else type="warning" size="small">未识别</el-tag>
              </template>
            </el-table-column>
            ...
            <el-table-column prop="sellerName" label="销售方" min-width="150" show-overflow-tooltip>
              <template #default="{ row }">
                <span v-if="row.sellerName" class="seller-name">{{ row.sellerName }}</span>
                <el-tag v-else type="warning" size="small">未识别</el-tag>
              </template>
            </el-table-column>

            <!-- 价税合计 -->
            <el-table-column prop="totalAmount" label="价税合计" min-width="120" sortable>
              <template #default="{ row }">
                <span v-if="row.totalAmount" class="total-amount">¥{{ row.totalAmount }}</span>
                <el-tag v-else type="warning" size="small">未识别</el-tag>
              </template>
            </el-table-column>
      ...
      <div v-else class="card-view">
        <div class="cards-grid" v-loading="parsing" loading-text="正在解析发票信息...">
          <div 
          ...
          >
            <!-- 发票预览图 -->
            <div class="invoice-preview" @click="handlePreviewClick(invoice.file)">
            ...
            <div class="invoice-info">
              ...
              <div class="invoice-details">
                <div class="detail-item">
                  <label>发票号码:</label>
                  <span v-if="invoice.invoiceNumber" class="value">{{ invoice.invoiceNumber }}</span>
                  <el-tag v-else type="warning" size="small">未识别</el-tag>
                </div>
                
                <div class="detail-item">
                  <label>开票日期:</label>
                  <span v-if="invoice.invoiceDate" class="value">{{ invoice.invoiceDate }}</span>
                  <el-tag v-else type="warning" size="small">未识别</el-tag>
                </div>
                
                <div class="detail-item">
                  <label>购买方:</label>
                  <span v-if="invoice.buyerName" class="value" :title="invoice.buyerName">
                ...
                <div class="detail-item">
                  <label>销售方:</label>
                  <span v-if="invoice.sellerName" class="value" :title="invoice.sellerName">
                ...
                <div class="detail-item amount-item">
                  <label>价税合计:</label>
                  <span v-if="invoice.totalAmount" class="amount-value">¥{{ invoice.totalAmount }}</span>
                  <el-tag v-else type="warning" size="small">未识别</el-tag>
                </div>
              </div>
              ...
              <!-- 展开的详细信息 -->
              <div v-if="expandedCards.includes(invoice.fileName)" class="expanded-details">
                <div class="expanded-grid">
                  ...
                  <div class="expanded-item">
                    <label>税额:</label>
                    <span>{{ invoice.taxAmount || '未识别' }}</span>
                  </div>
                  <div class="expanded-item">
                    <label>不含税金额:</label>
                    <span>{{ invoice.amountWithoutTax || '未识别' }}</span>
                  </div>
                  <div class="expanded-item">
                    <label>开票人:</label>
                  ...
                  <div class="expanded-item">
                    <label>收款人:</label>
                    <span>{{ invoice.payee || '未识别' }}</span>
                  </div>
                  <div class="expanded-item">
                    <label>复核人:</label>
                    <span>{{ invoice.reviewer || '未识别' }}</span>
                  </div>
                  <div class="expanded-item">
                    <label>项目名称:</label>
