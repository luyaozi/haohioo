...
const emit = defineEmits<{
  ...
  "duplicate-filtered": [fileName: string]
}>()
...
const handleFileChange = (file: any, fileList: any[]) => {
  ...
  // 检查是否为重复文件
  if (props.existingFiles.includes(file.name)) {
    ElMessage.warning(`文件 "${file.name}" 已存在，请勿重复上传`)
    
    // 发出重复文件过滤事件
    emit("duplicate-filtered", file.name)
    
    // 从文件列表中移除重复文件
    const filteredFileList = fileList.filter(f => f.uid !== file.uid)
    internalFileList.value = filteredFileList
    
    // 更新上传组件的文件列表
    if (uploadRef.value) {
      uploadRef.value.clearFiles()
      filteredFileList.forEach((f) => {
