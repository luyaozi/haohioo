    ...
    >
      <div v-if="selectedInvoice" class="w-full">
        <div class="flex items-center justify-between mb-6 pb-4 border-b border-gray-200">
          <h3 class="text-lg font-semibold text-gray-800 m-0">{{ selectedInvoice.fileName }}</h3>
          <div class="flex items-center gap-2">
            <el-button type="primary" @click="downloadRenamedFile(selectedInvoice)">
        ...
        <div class="space-y-6">
          <div class="bg-gray-50 rounded-lg p-4">
            <h4 class="text-base font-semibold text-gray-800 mb-4 m-0">发票信息</h4>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              <div class="flex items-center gap-2 text-sm py-1">
                <label class="font-medium text-gray-600 min-w-20 flex-shrink-0">发票号码:</label>
                <span class="text-gray-800 flex-1">{{ selectedInvoice.invoiceNumber || '未识别' }}</span>
              </div>
              <div class="flex items-center gap-2 text-sm py-1">
                <label class="font-medium text-gray-600 min-w-20 flex-shrink-0">开票日期:</label>
                <span class="text-gray-800 flex-1">{{ selectedInvoice.invoiceDate || '未识别' }}</span>
              </div>
              <div class="flex items-center gap-2 text-sm py-1">
                <label class="font-medium text-gray-600 min-w-20 flex-shrink-0">价税合计:</label>
                <span class="text-blue-600 font-medium flex-1">{{ selectedInvoice.totalAmount ? '¥' + selectedInvoice.totalAmount : '未识别' }}</span>
              </div>
              <div class="flex items-center gap-2 text-sm py-1">
                <label class="font-medium text-gray-600 min-w-20 flex-shrink-0">购买方:</label>
                <span class="text-gray-800 flex-1">{{ selectedInvoice.buyerName || '未识别' }}</span>
              </div>
              <div class="flex items-center gap-2 text-sm py-1">
                <label class="font-medium text-gray-600 min-w-20 flex-shrink-0">销售方:</label>
                <span class="text-gray-800 flex-1">{{ selectedInvoice.sellerName || '未识别' }}</span>
              </div>
              <div class="flex items-center gap-2 text-sm py-1">
                <label class="font-medium text-gray-600 min-w-20 flex-shrink-0">购买方税号:</label>
                <span class="text-gray-800 flex-1">{{ selectedInvoice.buyerTaxId || '未识别' }}</span>
              </div>
              <div class="flex items-center gap-2 text-sm py-1">
                <label class="font-medium text-gray-600 min-w-20 flex-shrink-0">销售方税号:</label>
                <span class="text-gray-800 flex-1">{{ selectedInvoice.sellerTaxId || '未识别' }}</span>
              </div>
              <div class="flex items-center gap-2 text-sm py-1">
                <label class="font-medium text-gray-600 min-w-20 flex-shrink-0">税额:</label>
                <span class="text-gray-800 flex-1">{{ selectedInvoice.taxAmount || '未识别' }}</span>
              </div>
              <div class="flex items-center gap-2 text-sm py-1">
                <label class="font-medium text-gray-600 min-w-20 flex-shrink-0">不含税金额:</label>
                <span class="text-gray-800 flex-1">{{ selectedInvoice.amountWithoutTax || '未识别' }}</span>
              </div>
              <div class="flex items-center gap-2 text-sm py-1">
                <label class="font-medium text-gray-600 min-w-20 flex-shrink-0">开票人:</label>
                <span class="text-gray-800 flex-1">{{ selectedInvoice.drawer || '未识别' }}</span>
              </div>
              <div class="flex items-center gap-2 text-sm py-1">
                <label class="font-medium text-gray-600 min-w-20 flex-shrink-0">收款人:</label>
                <span class="text-gray-800 flex-1">{{ selectedInvoice.payee || '未识别' }}</span>
              </div>
              <div class="flex items-center gap-2 text-sm py-1">
                <label class="font-medium text-gray-600 min-w-20 flex-shrink-0">复核人:</label>
                <span class="text-gray-800 flex-1">{{ selectedInvoice.reviewer || '未识别' }}</span>
              </div>
            </div>
          </div>
          
          <div class="bg-gray-50 rounded-lg p-4">
            <h4 class="text-base font-semibold text-gray-800 mb-4 m-0">原始文本</h4>
            <div class="bg-white border border-gray-200 rounded-md p-4 text-sm leading-relaxed text-gray-600 max-h-80 overflow-y-auto whitespace-pre-wrap break-all">{{ selectedInvoice.fullText || '无原始文本' }}</div>
          </div>
        </div>
      </div>
    </el-dialog>
...
:deep(.el-table) {
  ...
  // 确保固定列表头也有背景色
  .el-table__fixed-right .el-table__header-wrapper th,
  .el-table__fixed .el-table__header-wrapper th {
    background-color: #f8f9fa !important;
  }
