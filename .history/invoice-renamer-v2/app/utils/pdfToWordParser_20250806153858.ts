...
export class PDFToWordParser {
  ...
  async parseFile(file: File): Promise<ParsedInvoiceInfo> {
    ...
    try {
      ...
      const invoiceInfo = {
        ...basicInfo,
        ...Object.fromEntries(
          Object.entries(enhancedInfo).filter(
            ([_, value]) =>
              value && typeof value === "string" && value.trim() !== ""
          )
        ),
        ...Object.fromEntries(
          Object.entries(fourColumnInfo).filter(
            ([_, value]) =>
              value && typeof value === "string" && value.trim() !== ""
          )
        ),
  ...
  private sortTextItems(items: TextItem[]) {
    return TextProcessor.sortTextItems(items)
  }
  ...
  private reconstructText(items: TextItem[]): string {
    return TextProcessor.reconstructText(items)
  }
  ...
  private parseFourColumnStructure(text: string): Partial<ParsedInvoiceInfo> {
    ...
    lines.forEach((line, index) => {
      if (line) {
        ...
        if (foundKeywords.length > 0) {
          console.log(
            `📝 第${index}行包含关键词[${foundKeywords.join(", ")}]:`,
            JSON.stringify(line)
          )
    ...
    try {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        // 查找包含税号的独立行
        if (
          line.includes("统一社会信用代码") ||
          line.includes("纳税人识别号")
        ) {
          console.log("📝 找到税号相关行:", line)

          // 提取所有18位税号
          const taxIds = line.match(/[A-Z0-9]{18}/g)
          if (taxIds && taxIds.length > 0) {
            console.log("🔍 在税号行中找到税号:", taxIds)

            // 如果找到多个税号，第一个是购买方，第二个是销售方
            if (taxIds.length >= 1 && !result.buyerTaxId) {
              result.buyerTaxId = taxIds[0]
              console.log("✅ 设置购买方税号:", taxIds[0])
            }
            if (taxIds.length >= 2 && !result.sellerTaxId) {
              result.sellerTaxId = taxIds[1]
              console.log("✅ 设置销售方税号:", taxIds[1])
            }
          }
        }
        ...
        if (
          (line.includes("名称：") || line.includes("名 称:")) &&
          line.includes("\t") &&
          !line.includes("项目名称") &&
          !line.includes("规格型号")
        ) {
          ...
          for (let j = 0; j < columns.length; j++) {
            ...
            // 购买方名称列 - 支持多种格式
            if (
              (col.includes("购") || col.includes("买")) &&
              (col.includes("名称：") || col.includes("名 称:"))
            ) {
          ...
          if (
            sellerNameCol >= 0 &&
            !result.sellerName &&
            sellerNameCol + 1 < columns.length
          ) {
        ...
        if (
          ...
          // 提取购买方 - 支持多种格式
          let buyerMatch = line.match(/买 名 称\s*[:：]\s*([^\s售]+)/)
          if (!buyerMatch) {
            buyerMatch = line.match(/买名 称\s*[:：]\s*([^\s售]+)/)
          }
          if (buyerMatch && buyerMatch[1]) {
            result.buyerName = buyerMatch[1].trim()
          ...
          if (!sellerMatch) {
            // 尝试更宽松的匹配
            sellerMatch = line.match(/售 名 称\s*[:：]\s*(.+)/)
          }
          if (sellerMatch && sellerMatch[1]) {
            result.sellerName = sellerMatch[1].trim()
        ...
        // 处理单独的税号行（只包含18位税号的行）
        if (!result.sellerTaxId && line.match(/^\s*[A-Z0-9]{18}\s*$/)) {
          console.log("📝 找到单独的税号行:", line)
          const taxId = line.trim()
          if (taxId.length === 18) {
            result.sellerTaxId = taxId
            console.log("✅ 提取到销售方税号(单独行):", result.sellerTaxId)
          }
        }
        ...
        // 查找单独的购买方信息行
        if (
          (line.includes("购买方") || line.includes("买方")) &&
          !line.includes("项目名称") &&
          !line.includes("规格型号")
        ) {
        ...
        if (
          ...
          // 提取销售方名称
          const sellerNameMatch = line.match(
            /(?:销售方|卖方|售方)[：:\s]*([^\s\t]+)/
          )
      ...
      if (!result.buyerName || !result.sellerName) {
        ...
        for (let i = 0; i < lines.length; i++) {
          ...
          if (
            !result.sellerName &&
            (line.includes("销售方") || line.includes("卖方"))
          ) {
  ...
  private extractItemNameFromLines(lines: string[]): string {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (
        trimmedLine.includes("项目名称") ||
        trimmedLine.includes("货物或应税劳务") ||
        (trimmedLine.includes("*") &&
          (trimmedLine.includes("服务") || trimmedLine.includes("商品")))
      ) {
        itemStartIndex = i
        break
      }
