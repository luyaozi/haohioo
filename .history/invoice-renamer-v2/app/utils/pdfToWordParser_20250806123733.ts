...
export class PDFToWordParser {
  ...
  private async init() {
    ...
    try {
      // 如果静态worker已经初始化，直接使用
      if (PDFToWordParser.workerInitialized && PDFToWordParser.pdfjs) {
        this.getDocument = PDFToWordParser.pdfjs.getDocument
        this.initialized = true
        console.log('✅ PDFToWordParser 使用已初始化的worker')
        return
      }
      ...
      // 只有在worker未初始化时才设置
      if (!PDFToWordParser.workerInitialized) {
        await this.setupWorker(pdfjs)
        PDFToWordParser.workerInitialized = true
        PDFToWordParser.pdfjs = pdfjs
        console.log('✅ PDFToWordParser worker首次初始化成功')
      }
      
      this.getDocument = pdfjs.getDocument
      this.initialized = true
      console.log('✅ PDFToWordParser 初始化成功')
    } catch (error) {
      console.error('❌ PDFToWordParser 初始化失败:', error)
