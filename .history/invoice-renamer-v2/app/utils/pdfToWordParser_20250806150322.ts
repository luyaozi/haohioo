...
export class PDFToWordParser {
  ...
  private parseFourColumnStructure(text: string): Partial<ParsedInvoiceInfo> {
    ...
    // 输出包含购买方和销售方的文本片段用于调试
    const buyerIndex = text.indexOf('购买方')
    const sellerIndex = text.indexOf('销售方')
    if (buyerIndex !== -1) {
      const buyerSection = text.substring(Math.max(0, buyerIndex - 50), buyerIndex + 200)
      console.log('📝 购买方文本片段:', buyerSection)
    }
    if (sellerIndex !== -1) {
      const sellerSection = text.substring(Math.max(0, sellerIndex - 50), sellerIndex + 200)
      console.log('📝 销售方文本片段:', sellerSection)
    }
