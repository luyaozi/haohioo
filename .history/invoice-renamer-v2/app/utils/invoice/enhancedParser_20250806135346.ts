...
// 扩展的发票信息接口
interface ExtendedInvoiceInfo extends ParsedInvoiceInfo {
  buyerName?: string
  buyerTaxId?: string
  sellerName?: string
  sellerTaxId?: string
...
export class EnhancedInvoiceParser {
   ...
   * 综合解析所有字段
   */
  static parseAllFields(text: string): Partial<ParsedInvoiceInfo> {
    console.log('🔍 开始增强解析...')
    ...
    if (amountInWords) result.amountInWords = amountInWords
